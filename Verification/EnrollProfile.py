import VerificationServiceClientHelper
import sys
...
def enroll_profile(subscription_key, profile_id, file_path):
    ...
    """
    helper = VerificationServiceClientHelper.VerificationServiceClientHelper(
        subscription_key
    )
    ...
    print(
        "Enrollments Completed = {0}".format(
            enrollment_response.get_enrollments_count()
        )
    )
    print(
        "Remaining Enrollments = {0}".format(
            enrollment_response.get_remaining_enrollments()
        )
    )
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        print(
            "Usage: python EnrollProfile.py <subscription_key> <profile_id> "

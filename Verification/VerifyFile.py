...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        print(
            "Usage: python VerifyFile.py <subscription_key> <verification_file_path>"
            " <profile_id>..."
        )
        print("\t<subscription_key> is the subscription key for the service")
        print("\t<verification_file_path> is the audio file path for verification")
        print(
            "\t<profile_id> the ID for a profile to verify as the user using the submitted audio sample"
        )
